...
var getBestAxisDroppable = (function (_ref) {
  var isMovingOnMainAxis = _ref.isMovingOnMainAxis,
  ...
  var movementAxisStart = isMovingOnMainAxis ? axis.start : axis.crossAxisStart;
  var movementAxisEnd = isMovingOnMainAxis ? axis.end : axis.crossAxisEnd;
  var transverseAxisStart = isMovingOnMainAxis ? axis.crossAxisStart : axis.start;
  var transverseAxisEnd = isMovingOnMainAxis ? axis.crossAxisEnd : axis.end;
  var transverseAxisLine = isMovingOnMainAxis ? axis.crossAxisLine : axis.line;
  var isBetweenSourceClipped = isWithin(active[transverseAxisStart], active[transverseAxisEnd]);
  var candidates = toDroppableList(droppables).filter(function (droppable) {
    ...
    var activeOfTarget = getKnownActive(droppable);
    var isBetweenDestinationClipped = isWithin(activeOfTarget[transverseAxisStart], activeOfTarget[transverseAxisEnd]);
    return isBetweenSourceClipped(activeOfTarget[transverseAxisStart]) || isBetweenSourceClipped(activeOfTarget[transverseAxisEnd]) || isBetweenDestinationClipped(active[transverseAxisStart]) || isBetweenDestinationClipped(active[transverseAxisEnd]);
  }).sort(function (a, b) {
    var first = getKnownActive(a)[movementAxisStart];
    var second = getKnownActive(b)[movementAxisStart];
    ...
    return getKnownActive(droppable)[movementAxisStart] === getKnownActive(array[0])[movementAxisStart];
  });
  ...
  var contains = candidates.filter(function (droppable) {
    var isWithinDroppable = isWithin(getKnownActive(droppable)[transverseAxisStart], getKnownActive(droppable)[transverseAxisEnd]);
  ...
  if (contains.length > 1) {
    return contains.sort(function (a, b) {
      return getKnownActive(a)[transverseAxisStart] - getKnownActive(b)[transverseAxisStart];
    })[0];
  }
...
var moveAxis = (function (_ref) {
  var isMovingOnMainAxis = _ref.isMovingOnMainAxis,
      ...
      viewport = _ref.viewport,
      afterCritical = _ref.afterCritical;
  var destination = getBestAxisDroppable({
    isMovingOnMainAxis: isMovingOnMainAxis,
    isMovingForward: isMovingForward,
...
var moveInDirection = (function (_ref) {
  ...
  if (isMovingOnMainAxis) {
    var moveToNextPlaceResult = moveToNextPlace({
      isMovingForward: isMovingForward,
      previousPageBorderBoxCenter: previousPageBorderBoxCenter,
      draggable: draggable,
      destination: isOver,
