...
export default ({
  ...
  const movementAxisStart = isMovingOnMainAxis ? axis.start : axis.crossAxisStart;
  const movementAxisEnd = isMovingOnMainAxis ? axis.end : axis.crossAxisEnd;
  const transverseAxisStart =  isMovingOnMainAxis ?  axis.crossAxisStart : axis.start;
  const transverseAxisEnd = isMovingOnMainAxis ? axis.crossAxisEnd : axis.end;
  const transverseAxisLine = isMovingOnMainAxis ? axis.crossAxisLine : axis.line;
  
  const isBetweenSourceClipped = isWithin(active[transverseAxisStart], active[transverseAxisEnd]);
  ...
  if (contains.length > 1) {
    ...
    return contains.sort(
      (a: DroppableDimension, b: DroppableDimension): number =>
        getKnownActive(a)[transverseAxisStart] - getKnownActive(b)[transverseAxisStart],
    )[0];
  }
